    ...
    <div className="bg-gray-900 text-white min-h-screen p-6 font-sans">
      <div className="max-w-7xl mx-auto">
                ...
                <tr key={user.id} className="border-t border-gray-700 hover:bg-gray-700">
                  ...
                  <td className="py-2 px-4">
                    <label className="inline-flex items-center cursor-pointer">
                      ...
                      <div className="w-11 h-6 bg-gray-600 rounded-full peer peer-checked:bg-green-500 relative after:content-[''] after:absolute after:left-1 after:top-1 after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-full" />
                    </label>
                  </td>
                  <td className="py-2 px-4 space-x-2">
                    <button onClick={() => { setSelectedUser(user); setShowViewModal(true); }} className="text-blue-400 hover:text-blue-300 text-xs">View</button>
                    <button onClick={() => { setSelectedUser({ ...user }); setShowEditModal(true); }} className="text-yellow-400 hover:text-yellow-300 text-xs">Edit</button>
                    <button onClick={() => handleDeleteUser(user.id)} className="text-red-400 hover:text-red-300 text-xs">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="flex justify-between items-center mt-6 text-sm">
          <button onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} className="bg-gray-700 px-3 py-1 rounded" disabled={currentPage === 1}>Previous</button>
          <span className="text-gray-400">Page {currentPage} of {totalPages}</span>
          <button onClick={() => setCurrentPage(p => Math.min(p + 1, totalPages))} className="bg-gray-700 px-3 py-1 rounded" disabled={currentPage === totalPages}>Next</button>
        </div>

        {/* Add / View / Edit Modals */}
        {showModal && (
          <Modal title="Add New User" onClose={() => setShowModal(false)} onSave={handleAddUser} user={newUser} setUser={setNewUser} previewImage={previewImage} handleFileChange={handleFileChange} />
        )}
        {showViewModal && selectedUser && (
          <Modal title="User Details" readOnly user={selectedUser} onClose={() => { setSelectedUser(null); setShowViewModal(false); }} />
        )}
        {showEditModal && selectedUser && (
          <Modal title="Edit User" onClose={() => setShowEditModal(false)} onSave={handleEditUser} user={selectedUser} setUser={setSelectedUser} />
        )}
      </div>
    </div>
  );
...
const Modal = ({ title, onClose, onSave, user, setUser, previewImage, handleFileChange, readOnly = false }) => (
    ...
    <div className="bg-gray-800 p-6 rounded-lg w-full max-w-md">
      ...
      {["name", "email", "phone"].map((field, idx) => (
        <input
          ...
          value={user[field]}
          onChange={(e) => !readOnly && setUser && setUser({ ...user, [field]: e.target.value })}
          readOnly={readOnly}
      ...
      {!readOnly && setUser && (
        <>
          <select value={user.status} onChange={(e) => setUser({ ...user, status: e.target.value })} className="w-full mb-3 p-2 rounded bg-gray-700 text-white">
            <option value="Active">Active</option>
            <option value="Blocked">Blocked</option>
          </select>
          {handleFileChange && <input type="file" onChange={handleFileChange} className="w-full mb-3 p-2" />}
        </>
      )}
      {user.profilePicture || previewImage ? <img src={user.profilePicture || previewImage} alt="Preview" className="w-16 h-16 rounded-full mx-auto mb-3" /> : null}
      <div className="flex justify-end gap-2">
        <button onClick={onClose} className="bg-gray-600 px-4 py-2 rounded text-white">Close</button>
        {!readOnly && onSave && <button onClick={onSave} className="bg-blue-600 px-4 py-2 rounded text-white">Save</button>}
      </div>
    </div>
  </div>
);
